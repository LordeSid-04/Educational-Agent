import { Train, Home, Layers, GraduationCap, Settings, FolderOpen, FolderClosed, Hash, Mic, ArrowUp, Sparkles } from 'lucide-react';
    ...
    <div className="w-full h-full flex bg-ink overflow-hidden text-left select-none">
      ...
      <aside className="w-60 h-full glass-strong border-r border-white/10 flex flex-col shrink-0">
        <div className="h-14 flex items-center gap-2 px-4 border-b border-white/5">
          <span className="grid place-items-center w-7 h-7 rounded-lg bg-cyan text-ink"><Train size={15} /></span>
          <span className="font-display font-extrabold text-sm tracking-tight">CHALK<span className="text-cyan">MIND</span></span>
        </div>
        <div className="px-3 pt-3">
          <p className="font-mono text-[9px] tracking-[0.25em] uppercase text-cyan mb-2">Collections</p>
      ...
      <main className="flex-1 min-w-0 relative flex flex-col">
        <div className="h-14 flex items-center gap-2.5 px-6 shrink-0">
          ...
          <div>
            <p className="font-mono text-[9px] tracking-[0.25em] uppercase text-ash">Teaching Agent</p>
            <h1 className="font-display font-bold text-sm leading-none mt-0.5">Gradient Descent</h1>
          </div>
        </div>
        ...
        <div className="flex-1 overflow-hidden px-6 pb-2">
          <div className="mx-auto max-w-2xl relative rounded-2xl border-2 border-black bg-paper whiteboard-shadow h-full">
            ...
            <div className="relative p-6 text-ink">
              <h2 className="font-display font-black text-2xl border-b-4 border-ink pb-2">Gradient Descent</h2>
              <p className="text-ink/85 mt-3 text-[13px] leading-relaxed">
                Picture standing on a foggy hill, feeling the slope and stepping downhill toward the valley…
              </p>
              <div className="inline-flex items-center gap-2 bg-ink text-paper px-3 py-1 rounded-sm font-mono text-[9px] tracking-[0.2em] uppercase mt-4">The Core Idea</div>
              ...
              <div className="mt-4 grid grid-cols-3 gap-2.5">
                {['Input', 'Loss', 'Update'].map((b, i) => (
                  <div key={b} className={`h-12 grid place-items-center border-2 border-ink rounded font-bold text-xs ${i === 1 ? 'bg-acid' : ''}`}>{b}</div>
                ))}
              </div>
            </div>
          </div>
        ...
        <div className="px-6 pb-3">
          <div className="mx-auto max-w-2xl flex items-center gap-2 rounded-2xl glass-strong px-4 h-12">
            ...
            <span className="flex-1 text-ash text-sm font-mono">Ask the agent to teach you anything…</span>
